...
test_dataloader = dict(
    batch_size=6, num_workers=1,
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
        ann_file='annotations/test_vac_phase_2_coco.json',
        data_prefix=dict(img='test_vac_phase_2_images/'),
        test_mode=True,
        pipeline=test_pipeline,
    )
)
...
val_evaluator = dict(
    type='CocoMetric',
    ann_file=data_root + 'annotations/test_vac_phase_2_coco.json',
    metric='bbox',
    format_only=False)
